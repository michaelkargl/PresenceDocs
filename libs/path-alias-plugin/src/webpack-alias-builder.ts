import {Aliases, TsConfig} from "./models";
import {ITsconfigProvider} from "./tsconfig-provider.interface";
import path from 'path';

// TODO: refactor from tuple to object for better readability
type PathEntry = [alias: string, paths: string[]];

export class WebpackAliasBuilder {

    public constructor(
        private tsconfigProvider: ITsconfigProvider
    ) {
    }

    public async buildAsync(): Promise<Aliases> {
        const aliases = await this.convertTsconfigPathMappingsToWebpackAliasesAsync(this.tsconfigProvider);
        return aliases;
    }

    private async convertTsconfigPathMappingsToWebpackAliasesAsync(tsconfigProvider: ITsconfigProvider): Promise<Aliases> {
        ...
        pathEntries
            ...
            .forEach(([alias, paths]) => {
                ...
                const webpackPaths = paths.map(p => {
                    ...
                    const isRelativePath = !path.isAbsolute(globlessPath);
                    const absolutePath = isRelativePath
                        ? path.resolve(tsconfigProvider.tsconfigPath.directoryPath, globlessPath)
                        : globlessPath;
    ...
    private filterInvalidPathEntry(entry: PathEntry): boolean {
        return this.validatePathEntryPaths(entry) || this.validatePathEntryAlias(entry);
    }

    private validatePathEntryPaths(entry: PathEntry): boolean {
    ...
    private validatePathEntryAlias(entry: PathEntry): boolean {
        const [alias, paths] = entry;
